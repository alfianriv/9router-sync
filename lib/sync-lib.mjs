...
// Only external dep is better-sqlite3; we also try to reuse 9router's own copy
// so users don't need to install it themselves in the common case.
...
const HOME = homedir();
export const DB_PATH = join(HOME, '.9router', 'db', 'data.sqlite');
...
function loadBetterSqlite() {
  const candidates = [
    // Bundled/linked with this package (the normal install path).
    join(HERE, '..', 'node_modules', 'better-sqlite3'),
    // 9router ships its own prebuilt copy in its writable runtime dir.
    join(HOME, '.9router', 'runtime', 'node_modules', 'better-sqlite3'),
    // Global nvm install matching the currently running node.
    join(
      HOME,
      '.nvm',
      'versions',
    ...
    // Last resort: whatever node's resolver can find.
    'better-sqlite3',
  ];
  let lastErr;
...
export function openDb(readonly) {
  if (!existsSync(DB_PATH)) throw new Error(`9router db not found: ${DB_PATH}`);
